...
export default function ProductionVerifyPage() {
  ...
  async function submitAnswer(userAnswer) {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE}/api/session/submit-answer`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          phone: user.phone,
  ...
  async function finalizeSession() {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/session/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          phone: user.phone,
          session_id: sessionId,
          face_confidence: mockFaceConfidence,
          fraud_score: mockFraudScore,
          transcript: transcript
        })
      });
    ...
    <main className="px-4 py-8 sm:px-8 sm:py-10 lg:px-12">
      <section className="mx-auto w-full max-w-3xl space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-3 rounded-3xl card-clean px-6 py-4">
          ...
          <Link href="/" className="rounded-xl border border-[#1f4378]/20 bg-white px-4 py-2.5 text-sm font-bold text-[#1f4378] hover:border-[#1f4378]/45">
            Back to Dashboard
          </Link>
        </header>
