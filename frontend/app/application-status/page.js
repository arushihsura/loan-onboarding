...
export default function ApplicationStatusPage() {
  ...
  useEffect(() => {
    ...
    const fetchTimeline = async () => {
      try {
        const response = await fetch(`${API_URL}/api/application-timeline/${user.phone}`);
        if (!response.ok) {
  ...
  const getIconLarge = (iconType, color) => {
    switch(iconType) {
      case 'check':
        return <Check size={20} className={color} />;
      case 'circle':
        return <Circle size={20} className={color} />;
      case 'clock':
        return <Clock size={20} className={color} />;
      default:
        return <Circle size={20} className={color} />;
    }
  };
  ...
  return (
    <main className="px-4 py-8 sm:px-8 lg:px-12">
      <section className="mx-auto max-w-4xl space-y-8">
              ...
              <div key={idx} className="relative flex gap-6">
                ...
                <div className="relative flex flex-col items-center">
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center text-white ${
                    ...
                    {getIconLarge(item.icon, 'text-white')}
                  </div>
        ...
        {/* FAQs */}
        <div className="rounded-2xl border border-[#d7e3f8] bg-white p-8">
          <h3 className="title-font text-xl font-bold text-[#1b3155] mb-4">Common Questions</h3>
          <div className="space-y-4">
            <div>
              <p className="font-semibold text-[#1b3155]">When will I receive the funds?</p>
              <p className="text-sm text-[#5f7393] mt-1">After accepting the offer, funds will be transferred within 24 hours to your registered account.</p>
            </div>
            <div>
              <p className="font-semibold text-[#1b3155]">Can I modify the loan amount?</p>
              <p className="text-sm text-[#5f7393] mt-1">Yes, you can modify the amount during the verification process.</p>
            </div>
            <div>
              <p className="font-semibold text-[#1b3155]">What if I'm not approved?</p>
              <p className="text-sm text-[#5f7393] mt-1">We'll provide suggestions to improve your eligibility and try again in 30 days.</p>
            </div>
          </div>
        </div>
